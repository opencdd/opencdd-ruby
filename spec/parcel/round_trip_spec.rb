...
RSpec.describe "Parcel round-trip", :round_trip do
  # Round-trip guard: write a small workbook via Opencdd::Parcel::Writer,
  # read it back via Opencdd::Parcel::WorkbookReader, and assert the two
  ...
  it "round-trips classes through Parcel xlsx" do
    ...
    Dir.mktmpdir("cdd-round-trip") do |dir|
      ...
      Opencdd::Parcel::Writer.new(original).write(
        out,
        parcel_id: "ROUNDTRIP",
        project_id: "TEST",
        source_language: "en",
      ...
      reread = Opencdd::Reader.load_database(out)
