...
RSpec.describe "Import pipeline round-trips" do
  let(:source_database) { Cdd::Database.load_workbook(PARCEL_MAKER_XLSX.to_s) }
  ...
  def write_parcel(database, parcel_id:, **opts)
    ...
    Cdd::Parcel::Writer.new(database).write(path, parcel_id: parcel_id, **opts)
    path
  end

  describe "scenario 1: aggregate — multiple parcels → one Database" do
    it "merges two parcel exports into a single Database" do
      half1 = Cdd::Parcel.split(source_database, by: :entity_type)
      type_a = half1[:class]
      ...
      aggregated = Cdd::Parcel.aggregate(parcel_a, parcel_b)
    ...
    it "accepts a flattened variadic argument list" do
      partition = Cdd::Parcel.split(source_database, by: :entity_type)
      parcels = [
      ...
      aggregated = Cdd::Parcel.aggregate(parcels)
  ...
  describe "scenario 2: split — Database → partitioned parcels" do
    it "partitions by :entity_type into one Database per type" do
      partitions = Cdd::Parcel.split(source_database, by: :entity_type)
    ...
    it "partitions by :root_class into self-sufficient per-tree parcels" do
      partitions = Cdd::Parcel.split(source_database, by: :root_class)
      ...
      expect(sample).to be_a(Cdd::Database)
      # the root class is present in its own partition
      expect(sample.find_by_code(sample_root_code)).to be_a(Cdd::Klass)
    end

    it "partitions by :each_class into one Database per class" do
      partitions = Cdd::Parcel.split(source_database, by: :each_class)
      ...
      expect(sample_db.find(sample_class.irdi)).to be_a(Cdd::Klass)
    end

    it "rejects an unknown partition axis" do
      expect { Cdd::Parcel.split(source_database, by: :nonsense) }
  ...
  describe "scenario 3: parcel → cddal → parcel (round-trip)" do
    ...
    let(:parcel_source) do
      ...
      Cdd::Database.load_workbook(path)
    end

    it "preserves the entity graph through a CDDAL detour" do
      cddal_text = Cdd::Cddal.serialize(parcel_source)
      via_cddal = Cdd::Cddal.parse(cddal_text)

      parcel_path = write_parcel(via_cddal, parcel_id: "OCDDRT3")
      reloaded = Cdd::Database.load_workbook(parcel_path)
  ...
  describe "scenario 4: cddal → parcel → cddal (round-trip)" do
    let(:source_cddal) { Cdd::Cddal.parse_file(REFERENCE_DOCS.join("examples/oceanrunner.cddal")) }

    it "preserves semantic equality through a parcel detour" do
      parcel_path = write_parcel(source_cddal, parcel_id: "OCDDRT4")
      via_parcel = Cdd::Database.load_workbook(parcel_path)

      cddal_again = Cdd::Cddal.serialize(via_parcel)
      reloaded = Cdd::Cddal.parse(cddal_again)
  ...
  describe "scenario 5: import a parcel into an existing Database (upsert)" do
    ...
    it "raises TypeError when the CR is not a Database" do
      expect { source_database.apply_change_request("not a database") }
        .to raise_error(TypeError, /apply_change_request expects a Cdd::Database/)
    end
  end
