...
RSpec.describe Opencdd::Database, "#add_dictionary" do
  let(:db) { Opencdd::Database.new }

  it "creates a workbook with Class and Property sheets by default" do
    dict = Opencdd::Database::Dictionary.new(
      parcel_id: "OCDD1",
      source_language: "en",
    )
    wb = db.add_dictionary(dict)
    expect(wb).to be_a(Opencdd::Parcel::Workbook)
  ...
  it "populates sheetmap with pcls_LOCAL and per-sheet rows" do
    wb = db.add_dictionary(Opencdd::Database::Dictionary.new(parcel_id: "OCDD1"))
  ...
  it "records the project info on the workbook" do
    wb = db.add_dictionary(Opencdd::Database::Dictionary.new(
      parcel_id: "OCDD1",
      source_language: "fr",
  ...
  it "rejects invalid parcel IDs" do
    expect {
      db.add_dictionary(Opencdd::Database::Dictionary.new(parcel_id: "bad id"))
    }.to raise_error(ArgumentError, /invalid parcel_id/)
    expect {
      db.add_dictionary(Opencdd::Database::Dictionary.new(parcel_id: "lower"))
    }.to raise_error(ArgumentError, /invalid parcel_id/)
  end
