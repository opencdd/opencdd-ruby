...
module Cdd
  module Parcel
    ...
    #
    # For per-class sharded subdirs see +Cdd::Parcel::ShardedDirReader+.
    # For single .xlsx workbooks see +Cdd::Parcel::WorkbookReader+.
    class FlatDirReader
      ...
      def read_one(file)
        ...
        rows = read_rows(file)
        Cdd::Parcel::Sheet.from_rows(rows.each, name: sheet_name)
      end
      ...
      def sheetmap_entry_for(file)
        ...
        type = TYPE_BY_PREFIX[prefix]
        return nil unless type
        meta_class_code = Cdd::Parcel::TYPE_TO_META_CLASS[type]
        return nil unless meta_class_code

        Cdd::Parcel::Workbook::SheetMapEntry.new(
          project_id: "LOCAL",
          parcel_id: nil,
          class_irdi: Cdd::IRDI.parse("0112/2///62656_1##{meta_class_code}"),
          content_no: 0,
          sheet_no: nil,
          sheet_name: File.basename(file, ".*"),
          type: prefix,
