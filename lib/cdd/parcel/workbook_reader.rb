...
module Cdd
  module Parcel
    ...
    # legacy single-sheet .xls) and produces a +Cdd::Parcel::Workbook+.
    #
    # This is the canonical IEC 62656-1 Parcel container: one file
    # with Project / sheetmap / pcls_LOCAL / data sheets. Use
    # +Cdd::Parcel::FlatDirReader+ for the legacy 6-file-per-directory
    # layout, and +Cdd::Parcel::ShardedDirReader+ for the per-class
    # sharded layout.
    class WorkbookReader
      ...
      META_CLASS_BY_LEGACY_TYPE = Cdd::MetaClasses::TYPE_BY_META_CLASS.invert.freeze
      ...
      def read_workbook
        ...
        workbook.sheet_names.each do |sheet_name|
          ...
          sheet = Cdd::Parcel::Sheet.from_rows(workbook.rows_for(sheet_name).each, name: sheet_name)
          sheets << sheet if sheet
        end

        if sheets.empty? && legacy?
          sheets = [Cdd::Parcel::Sheet.from_rows(
          ...
          sheetmap = sheets.map { |s|
            ...
            Workbook::SheetMapEntry.new(
              ...
              class_irdi: Cdd::IRDI.parse("0112/2///62656_1##{META_CLASS_BY_LEGACY_TYPE[type]}"),
              content_no: 0,
              sheet_no:   nil,
              sheet_name: s.name,
      ...
      def parse_sheetmap(rows)
        ...
        rows[1..].map do |r|
          ...
          Workbook::SheetMapEntry.new(
            ...
            class_irdi: h[:classid] && Cdd::IRDI.parse(h[:classid]),
