  ...
  module Parcel
    class Sheet
      ...
      def self.scaffold(meta_class_irdi:, parcel_id:, source_language: DEFAULT_SOURCE_LANGUAGE,
        ...
        meta = Cdd::MetaClasses.for(meta_class_irdi.to_s) or
          raise ArgumentError, "unknown meta-class #{meta_class_irdi}"

        code_property_id = Cdd::MetaClasses.code_property_id_for(meta.irdi)
      ...
      def self.from_rows(rows, name: nil)
        metadata = Cdd::Parcel::Metadata.new
        header_rows = []
        data_rows = []
        ...
        schema = Cdd::Parcel::SheetSchema.from_header_rows(header_rows)
