  ...
  module Validator
    class TypeRule < Rule
      SIMPLE_PREDICATES = {
        ...
        "IRDI_TYPE"                 => ->(v) { !Cdd::IRDI.parse(v.to_s).nil? },
        "IRDI_STRING_TYPE"          => ->(v) { !Cdd::IRDI.parse(v.to_s).nil? },
        "ICID_STRING"               => ->(v) { !Cdd::IRDI.parse(v.to_s).nil? },
        "ICID_STRING_TYPE"          => ->(v) { !Cdd::IRDI.parse(v.to_s).nil? },
        "DATE_TYPE"                 => ->(v) { date?(v) },
