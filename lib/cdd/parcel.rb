...
module Cdd
  module Parcel
    META_CLASS_TYPES = Cdd::MetaClasses::TYPE_BY_META_CLASS
    TYPE_TO_META_CLASS = META_CLASS_TYPES.invert.freeze

    autoload :Metadata,        "cdd/parcel/metadata"
    autoload :SheetSchema,     "cdd/parcel/sheet_schema"
    autoload :Sheet,           "cdd/parcel/sheet"
    autoload :Workbook,        "cdd/parcel/workbook"
    autoload :WorkbookReader,  "cdd/parcel/workbook_reader"
    autoload :FlatDirReader,   "cdd/parcel/flat_dir_reader"
    autoload :ShardedDirReader, "cdd/parcel/sharded_dir_reader"
    autoload :ReferencedIrdis,  "cdd/parcel/referenced_irdis"
    autoload :Selector,        "cdd/parcel/selector"
    autoload :SheetEmitter,    "cdd/parcel/sheet_emitter"
    autoload :Writer,          "cdd/parcel/writer"
    autoload :CsvWriter,       "cdd/parcel/csv_writer"
    autoload :CsvReader,       "cdd/parcel/csv_reader"
    autoload :ScrapeVerifier,  "cdd/parcel/scrape_verifier"
    ...
    # polymorphically via +Cdd::Reader.detect+ and merged into a
    # single target database.
    #
    # Returns the populated Cdd::Database.
    def aggregate(*paths)
      paths.flatten
        .map { |p| Cdd::Database.load(p) }
        .reduce(Cdd::Database.new) { |acc, db| acc.merge(db) }
    end
        ...
        if lift_dependencies
          ...
          klass_props.each do |prop|
            ...
            if prop.unit_irdi
              ...
              entities << unit if unit.is_a?(Cdd::Unit)
            end
    ...
    def lift_for(database, classes)
      ...
      classes.each do |klass|
        ...
        database.properties_of(klass).each do |prop|
          ...
          if prop.unit_irdi
            unit = database.find(prop.unit_irdi)
            entities << unit if unit.is_a?(Cdd::Unit) && !entities.include?(unit)
          end
          vl = database.value_list_of(prop)
